...
- `cxsign account add 12345678910`, 添加账号。会提示输入密码。
  ...
  > **请注意账号安全**：输入密码时不会在命令行界面显示任何字符，但是密码依然会被**明文**缓存在[配置目录](#其他)下的数据库文件 `cx.db` 当中。
  > 请注意保护登录者的账号密码。
  > > - 如果让他人登录你的账号，请仔细甄别其是否值得信任。
  > > - 如果你登录了其他人的账号，请牢记：能力越大，责任越大，同时不要辜负他人对你的信任。
  > > - 你的账号不论供自己登录还是供他人登录，都推荐为其设置一个单独的、别于其他账号甚至完全无关的、不常用的密码，防止该密码被盗取后用于登录其他平台账号或猜出你设置密码的风格；
