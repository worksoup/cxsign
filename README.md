...
### 签到相关
- `cxsign`, 处理所有**有效**签到活动。或：`cxsign 2000024077709`, 处理 `active_id` 为 `2000024077709` 的签到活动。
...
#### "validate"
##### 原因
~~签到失败返回的消息之一，应该是登录 ip 的问题，但是用官方客户端似乎不会触发该问题。造成该区别的原因我并未从互联网上找到公开资料，可能是因为我没有设置官方客户端的 UA?~~

~~似乎和 UA 无关，需要继续研究。~~

与预签到和签到之间的提交延迟有关，现已修复。详情请参考[这里](https://github.com/kuizuo/chaoxing-sign/issues/31)。

##### 解决方法
现已修复。如果依然报错，请尝试在同一网络环境下使用官方客户端登录并打开一下签到（打开即可，不需要尝试签到。这应该是相当于手动进行了一次预签到）。
